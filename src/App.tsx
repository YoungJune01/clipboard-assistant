...
function QuickPanel({ commands, text, language }: { commands: AppCommands; text: Dictionary; language: Language }) {
  ...
  const createAndAssignGroup = async (id: string, name: string) => { const value = Array.from(name.trim()).slice(0, 30).join(""); if (!value) return false; try { const group = await commands.createClipboardGroup(value); setGroups((items) => [...items, group]); await changeRecordGroup(id, group.id); return true; } catch { setStatus("groupSaveFailed"); return false; } };
...
function ClipboardItem({ record, index, selected, groups, itemRef, onSelect, onPaste, onCopyText, onOpenUrl, onDelete, onTogglePinned, onToggleFavorite, onEdit, onSaveNote, onChangeGroup, onCreateGroup, loadImagePreview, text, language }: { record: SessionRecord; index: number; selected: boolean; groups: ClipboardGroup[]; itemRef(node: HTMLElement | null): void; onSelect(): void; onPaste(): void; onCopyText(value: string): void; onOpenUrl(url: string): void; onDelete(): void; onTogglePinned(): void; onToggleFavorite(): void; onEdit(): void; onSaveNote(note: string): Promise<boolean>; onChangeGroup(groupId: string): Promise<void>; onCreateGroup(name: string): Promise<boolean>; loadImagePreview(): Promise<ImagePreview>; text: Dictionary; language: Language }) {
  ...
  useEffect(() => {
    if (!groupMenu) return;
    const close = (event: PointerEvent) => { if (!groupMenuRef.current?.contains(event.target as Node)) setGroupMenu(null); };
    const escape = (event: KeyboardEvent) => { if (event.key === "Escape") setGroupMenu(null); };
  ...
  const fileSummary = record.contentKind === "files" ? describeFiles(record.filePaths, record.fileCount, text) : null;
  ...
  const finishNote = async () => { if (note !== (record.note ?? "")) await save(note); setEditingNote(false); };
  const chooseGroup = async (groupId: string) => { await onChangeGroup(groupId); setGroupMenu(null); };
  const createGroup = async () => { if (await onCreateGroup(newGroupName)) { setNewGroupName(""); setCreatingGroup(false); setGroupMenu(null); } };
  const noteTone = Array.from(record.id).reduce((total, character) => total + character.charCodeAt(0), 0) % 4;
  return <article ref={itemRef} id={`clipboard-record-${record.id}`} className={`clipboard-item${selected ? " selected" : ""}${record.pinned ? " pinned" : ""}`} role="option" tabIndex={0} aria-selected={selected} onClick={onSelect} onDoubleClick={onPaste} onContextMenu={(event) => { event.preventDefault(); event.stopPropagation(); onSelect(); setCreatingGroup(false); setNewGroupName(""); setGroupMenu({ x: Math.min(event.clientX, window.innerWidth - 188), y: Math.min(event.clientY, window.innerHeight - Math.min(250, 74 + groups.length * 31)) }); }} onKeyDown={(event) => { if (event.key === "Enter" && !editingNote && !groupMenu) { event.preventDefault(); onPaste(); } }}><div className="item-index">{index + 1}</div><div className="item-content"><div className="item-meta"><span>{record.sourceApplication ?? text.unknownApp}</span>{webUrl && <span className="content-type" aria-label={`${text.webLink}: ${webUrl.hostname}`}><Link2 size={11} aria-hidden="true" />{webUrl.hostname}</span>}<time>{formatTime(record.capturedAt, language, text.now)}</time>{record.hasImage && <Image size={13} aria-label={text.containsImage} />}<div className="record-actions">{!note && <button type="button" aria-label={text.addNote} title={text.addNote} onClick={action(() => setEditingNote(true))} onDoubleClick={stopDoubleClick}><MessageSquarePlus size={12} /></button>}<button className={record.pinned ? "active" : ""} type="button" aria-label={record.pinned ? text.unpinRecord : text.pinRecord} title={record.pinned ? text.unpinRecord : text.pinRecord} onClick={action(onTogglePinned)} onDoubleClick={stopDoubleClick}><Pin size={12} /></button><button className={record.favorite ? "active favorite" : ""} type="button" aria-label={record.favorite ? text.unfavoriteRecord : text.favoriteRecord} title={record.favorite ? text.unfavoriteRecord : text.favoriteRecord} onClick={action(onToggleFavorite)} onDoubleClick={stopDoubleClick}><Heart size={12} fill={record.favorite ? "currentColor" : "none"} /></button>{record.contentKind === "text" && <button type="button" aria-label={text.editRecord} title={text.editRecord} onClick={action(onEdit)} onDoubleClick={stopDoubleClick}><Pencil size={12} /></button>}<button className="danger" type="button" aria-label={text.deleteRecord} title={text.deleteRecord} onClick={action(onDelete)} onDoubleClick={stopDoubleClick}><Trash2 size={12} /></button></div></div>{record.hasImage && <div className={`image-preview${imagePreviewFailed ? " failed" : ""}`} aria-label={imagePreview ? text.imagePreview : imagePreviewFailed ? text.imagePreviewUnavailable : text.imagePreviewLoading}>{imagePreview ? <img src={imagePreview.dataUrl} width={imagePreview.width} height={imagePreview.height} alt={text.imagePreview} /> : <Image size={22} aria-hidden="true" />}</div>}{fileSummary && <div className="file-summary" aria-label={fileSummary.accessibleLabel}><File size={18} aria-hidden="true" /><div><strong>{fileSummary.title}</strong><span>{fileSummary.detail}</span>{fileSummary.more && <small>{fileSummary.more}</small>}</div></div>}{record.text && (webUrl ? <button className="item-text web-url interactive-text" type="button" aria-label={`${text.openLink}: ${webUrl.hostname}`} title={text.openLink} onClick={action(() => onOpenUrl(webUrl.href))} onDoubleClick={stopDoubleClick}>{record.text}<ExternalLink size={12} aria-hidden="true" /></button> : <p className="item-text">{record.text}</p>)}{record.qrText && <RecognitionResult title={text.qrContent} value={record.qrText} url={qrUrl} copyLabel={text.copyQrContent} text={text} onCopy={onCopyText} onOpenUrl={onOpenUrl} />}{record.ocrText && <RecognitionResult title={text.recognizedText} value={record.ocrText} url={ocrUrl} copyLabel={text.copyOcrText} text={text} onCopy={onCopyText} onOpenUrl={onOpenUrl} />}{!fileSummary && !record.text && !record.qrText && !record.ocrText && <p className="item-text">{record.hasImage ? text.imageItem : text.clipboardItem}</p>}{editingNote ? <input autoFocus className="note-editor" value={note} placeholder={text.addNote} aria-label={text.noteFor(description)} onClick={(event) => event.stopPropagation()} onDoubleClick={stopDoubleClick} onChange={(event) => update(event.currentTarget.value)} onKeyDown={(event) => { event.stopPropagation(); if (event.key === "Enter") { event.preventDefault(); skipBlur.current = true; void finishNote(); event.currentTarget.blur(); } if (event.key === "Escape") { skipBlur.current = true; update(record.note ?? ""); setEditingNote(false); event.currentTarget.blur(); } }} onBlur={() => { if (skipBlur.current) { skipBlur.current = false; return; } void finishNote(); }} /> : note ? <button className={`note-chip note-tone-${noteTone}`} type="button" aria-label={text.noteFor(description)} title={text.editNote} onClick={action(() => setEditingNote(true))} onDoubleClick={stopDoubleClick}>{text.notePrefix}: {note}</button> : null}</div>{groupMenu && <div ref={groupMenuRef} className="record-group-menu" role="menu" aria-label={text.addToGroup} style={{ left: groupMenu.x, top: groupMenu.y }} onClick={(event) => event.stopPropagation()} onDoubleClick={stopDoubleClick}><div className="record-group-menu-title"><FolderInput size={13} />{text.addToGroup}</div><button type="button" role="menuitemradio" aria-checked={!record.groupId} onClick={() => void chooseGroup("")}>{!record.groupId && <Check size={13} />}{text.ungrouped}</button>{groups.map((group) => <button key={group.id} type="button" role="menuitemradio" aria-checked={record.groupId === group.id} onClick={() => void chooseGroup(group.id)}>{record.groupId === group.id && <Check size={13} />}{group.name}</button>)}<div className="record-group-menu-separator" />{creatingGroup ? <form className="record-group-create" onSubmit={(event) => { event.preventDefault(); void createGroup(); }}><input autoFocus value={newGroupName} maxLength={30} aria-label={text.groupName} placeholder={text.groupName} onChange={(event) => setNewGroupName(event.currentTarget.value)} onKeyDown={(event) => { if (event.key === "Escape") { event.stopPropagation(); setCreatingGroup(false); } }} /><button type="submit" aria-label={text.create} title={text.create} disabled={!newGroupName.trim()}><Check size={13} /></button></form> : <button type="button" role="menuitem" onClick={() => setCreatingGroup(true)}><FolderPlus size={13} />{text.createAndAddGroup}</button>}</div>}</article>;
}

function describeFiles(paths: string[], count: number, text: Dictionary) {
  const names = paths.map((path) => { const parts = path.split(/[\\/]/).filter(Boolean); return parts[parts.length - 1] ?? path; });
  const firstPath = paths[0] ?? "";
  const firstName = names[0] ?? text.fileClipboardItem;
  const parentParts = firstPath.replace(/[\\/]+$/, "").split(/[\\/]/).slice(0, -1);
  const parent = parentParts[parentParts.length - 1];
  const extensionParts = firstName.split(".");
  const extension = firstName.includes(".") && !firstName.endsWith(".") ? extensionParts[extensionParts.length - 1]?.toUpperCase() : null;
  const total = Math.max(count, names.length, 1);
  return {
    title: total > 1 ? text.fileTitle(firstName, total - 1) : firstName,
    detail: [extension ? text.fileType(extension) : text.fileItem, parent ? text.fileLocation(parent) : null].filter(Boolean).join(" · "),
    more: names.slice(1, 3).join(" · ") || null,
    accessibleLabel: text.fileSummary(firstName, total),
  };
}
