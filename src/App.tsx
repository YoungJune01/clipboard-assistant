...
function QuickPanel({ commands, text, language }: { commands: AppCommands; text: Dictionary; language: Language }) {
  ...
  useEffect(() => { if (!status) return; const timeout = window.setTimeout(() => setStatus(null), status === "pasteSent" ? PASTE_HINT_DURATION_MS : ERROR_HINT_DURATION_MS); return () => window.clearTimeout(timeout); }, [status]);
  ...
  const isError = historyError || status === "pasteFailed" || status === "noteSaveFailed" || status === "groupSaveFailed" || status === "deleteFailed";
...
function detectWebUrl(value: string | null) {
  ...
  try {
    ...
    return { hostname: parsed.hostname.replace(/^www\./i, "") };
  } catch {
    return null;
  }
}
